...
static void parse_arguments(int argc, char **argv, 
        ...
        const char **iname, const char **oname, bool *print_header);
...
int main(int argc, char **argv) {
    ...
    parse_arguments(argc, argv, &storage_mode, &iname, &oname,&print_header);
    ...
    while ((value = ply_get_next_obj_info(iply, value)))
        if (!ply_add_obj_info(oply, value))
            error("Failed adding comments");
    /* write output header default, if option _wh specified, doesn't write it */
	if(print_header==true){
   	 if (!ply_write_header(oply)) error("Failed writing '%s' header", oname);
   	 }else ;//do nothing : don't need to write header
...
static void help(void) {
    error("Usage:\n"  
            ...
            "    -a_nh, --ascii_nh: convert to ascii format wihtout text header\n");
}
...
static void parse_arguments(int argc, char **argv, 
        e_ply_storage_mode *storage_mode, 
        const char **iname, const char **oname, bool *print_header) {
        *print_header=true; //default behavior
    if (argc < 4) help();
    ...
    else if (strcmp(argv[1], "--ascii_nh") == 0 || 
            strcmp(argv[1], "-a_nh") == 0){
            *storage_mode = PLY_ASCII;
            *print_header=false;}
    else help(); 
    *iname = argv[2];
